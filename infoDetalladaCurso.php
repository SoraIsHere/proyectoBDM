...
$cursoQuery = "CALL ObtenerInformacionCurso(?)";
$stmt = $conexion->prepare($cursoQuery);
...
$leccionesQuery = "CALL ObtenerLeccionesPorCurso(?)";
$stmt = $conexion->prepare($leccionesQuery);
$stmt->bind_param('i', $cursoID);
$stmt->execute();
$leccionesResult = $stmt->get_result();
?>
...
<body>
    <main style="margin: 100px 0 30px">
        ...
        <form method="post">
            ...
            <input type="text" id="nombre" name="nombre" value="<?php echo htmlspecialchars($curso['Nombre']); ?>"
                required>
            <label for="descripcion">Descripción:</label>
            <textarea id="descripcion" name="descripcion"
                required><?php echo htmlspecialchars($curso['Descripcion']); ?></textarea>
            <label for="costoGeneral">Costo General:</label>
            <input type="number" id="costoGeneral" name="costoGeneral"
                value="<?php echo htmlspecialchars($curso['CostoGeneral']); ?>" required>
            <button type="submit" name="update_curso">Guardar cambios</button>
            <button type="submit" name="delete_curso"
                onclick="return confirm('¿Estás seguro de que deseas eliminar este curso?');">Eliminar curso</button>
        </form>
                ...
                <tbody>
                    <?php while ($leccion = $leccionesResult->fetch_assoc()): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <button
                                    data-leccion='<?php echo json_encode($leccionDatos, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>'
                                    onclick="mostrarModal(this.getAttribute('data-leccion'))">Detalles</button>
                            </td>
                        </tr>
                    <?php endwhile; ?>
        ...
        <div id="modal" class="modal">
            <div class="modal-content">
                ...
                <form method="post">
                    ...
                    <button type="submit" name="delete_leccion"
                        style="background-color: red; color: white;">Eliminar</button>
                </form>
            </div>
        </div>
        <script>
            function mostrarModal(leccionData) {
                var leccion = JSON.parse(leccionData);
                console.log("Lección recibida:", leccion);  // Verificación de qué se está recibiendo
                if (leccion && leccion.LeccionID) {
                    ...
                    console.error("Error: lección no definida o falta LeccionID.");
                }
            }
